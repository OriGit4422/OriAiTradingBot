    ...
    const [results, goldResults] = await Promise.all([
      ...
      Promise.allSettled(
        goldTimeframes.map(async (tf) => {
          ...
          return {
            coin: 'XAUUSD', type: g.type === 'BUY' ? 'LONG' : 'SHORT',
            entry: g.entry, tp: g.tp, sl: g.sl, marketPrice: g.entry,
            timeframe: g.timeframe, confidence: g.confidence,
            strategy: 'Gold AI Strategy', rrRatio: g.rrRatio, isGold: true,
            goldReasoning: g.reasoning, trend: g.trend, keyLevels: g.keyLevels,
            indicators: {
